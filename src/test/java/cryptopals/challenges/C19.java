 ...
 *
 * In successive encryptions (not in one big running CTR stream), encrypt each line of the base64 decodes of the following,
 * producing multiple independent ciphertexts (see texts in class)
 *
 ...
 *
 * Attack this cryptosystem piecemeal: guess letters, use expected English language frequency to validate guesses,
...
public class C19 {
    ...
    @Test
    void challenge19() {
        final byte[][] encryptedTexts = new byte[TEXTS.length][];
        final CTR ctr = new CTR(ByteArrayUtil.randomBytes(16,
                "L'ÉTAT EST CETTE GRANDE FICTION PAR LAQUELLE TOUT LE MONDE ESSAIE DE VIVRE AUX DÉPENS DE TOUT LE MONDE"));
        final Challenge19Tool tool = new Challenge19Tool();

        for (int i = 0; i < TEXTS.length; i++) {
            //encrypt each text and store in the result array
            encryptedTexts[i] = ctr.encrypt(new String(Base64.getDecoder().decode(TEXTS[i])));
        }
