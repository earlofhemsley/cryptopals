...
public class PKCS1V15RsaSignatureUtil {
    ...
    public String sign(final String message, final RSA.Key privateKey) {
        ...
        final byte[] asn1PlusHash = ASN1Util.encodeHashToAsn1SignatureFormat(hash, X509ObjectIdentifiers.id_SHA1);
    ...
    public boolean verifySignature(final String message, final String base64Signature, final RSA.Key publicKey) {
        ...
        try {
            ...
            if (!oid.getAlgorithm().equals(X509ObjectIdentifiers.id_SHA1)) {
                log.error("didn't detect sha1 ({}) as the OID. Instead detected {}", X509ObjectIdentifiers.id_SHA1.getId(),
